...
parser = argparse.ArgumentParser(description='Add datasets to pre-existing vocab file using provided targets')
...
if len([i for i in args_list if i == '-f']) > 1:
    print(f'{RED}Currently can only add one dataset at a time{NC}')
    sys.exit(1)

args_list += ['-n', str(num), '-m', str(max_len).lower(), '-o', str(override).lower(), 
              '-v', vocab, '-s', str(sysimage).lower()]

dataset_backbone.generate_dataset(args, aggregate_args=aggregate_args, do_full_processing=True)
...
print(f'{GREEN}Finished generating dataset with pre-existing vocab file{NC}')
